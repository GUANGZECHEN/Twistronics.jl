...
n_a1=90
n_a2=90
m=3          # in version 0.1.0, when m is even, it does not work: the way the I construct the lattice destroys bipartite nature of honeycomb (will fix in later versions)
r=1
...
momenta, energies, layer_polarization=get_band_twisted_layer(H,k_points,n_bands) # get bandstructure with valley-polarization
np.savetxt("bands.OUT",np.transpose([momenta, energies, layer_polarization]))

# plot bandstructure
plt.figure(figsize=(6,6),dpi=80)
colors=[(1,0,0),(0,1,0),(0,0,1)]
cm=colormap.LinearSegmentedColormap.from_list("my_list", colors) 
sc=plt.scatter(momenta, energies,c=layer_polarization,cmap=cm,vmin=-1,vmax=1)  # color indicates valley polarization
plt.colorbar(sc)
